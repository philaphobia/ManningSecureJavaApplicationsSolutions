...
public class ServletHandler extends HttpServlet {
	 ...
	 * 
	 * SOLUTION: The scope of this loginEmail variable here is within the
	 * singleton class of this HttpServlet which means it could be leaked across
	 * sessions. The variable scope and context should not be used here. It
	 * should be commented out and fixed within the context below in the
	 ...
	 * System.err.println("Missing argument"); System.exit(1); }
	 * 
	 * switch(args[0]) { case "database": try { Connection connection =
	 * DB.getDbConnection(null); try (PreparedStatement stmt =
	 * connection.prepareStatement("SELECT COUNT(*) FROM tasks") ) { //do
	 * nothing } } catch(DBException dbe) { dbe.printStackTrace(); }
	 * catch(SQLException sqe) { sqe.printStackTrace(); }
	 * 
	 * break;
	 * 
	 * default: System.err.println("Function " + args[0] + " not implemented");
	 * System.exit(1); } }
	 */
	// SOLUTION END
		...
		switch (action) {
			...
			case "file_upload" :
				try {
					...
					AppLogger.log(
							"POST caught IOException: " + ioe.getMessage());
					response.setStatus(
							HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
			...
			default :
				 ...
				 * requests are processed such as by POST or GET. Unclear
				 ...
				 * request on to the GET. The forwarding to doGet() needs to be
				 * removed and and error returned to the user since no matching
				 * logic was made at this point.
				 * 
				 * The following lines are commented out from the original code:
				 *
				 * //nothing matched so process as a GET AppLogger.log("Cannot
				 * process POST request, forwarding to GET"); doGet(request,
				 * response);
				 * 
				 * break;
				 */
	...
	protected void doGet(HttpServletRequest request,
		...
		// throw ServletException for processing
			...
			AppLogger
					.log("POST caught AppException: " + ae.getPrivateMessage());
			/*
			 * SOLUTION: Logging the error is correct, but throwing
			 * ServletException is not the correct flow. An error should be sent
			 * to the user via the HttpResponse with a 500 error with
