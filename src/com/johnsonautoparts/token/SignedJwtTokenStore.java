...
public class SignedJwtTokenStore implements TokenStore {
	...
	public SignedJwtTokenStore(HttpServletRequest request, String audience)
			throws AppException {
		...
		if (secretObj instanceof byte[]) {
			throw new AppException(
					"SignedJwtTokenStore cannot find secret in session");
		}
		...
		try {
			...
			throw new AppException(
					"SignedJwtTokenStore received key with wrong length: "
							+ kle.getMessage());
		} catch (JOSEException je) {
			throw new AppException("SignedJwtTokenStore caught JOSEException: "
					+ je.getMessage());
		}
	...
	public Token createToken(String username) throws AppException {
		try {
			...
			return new Token(Instant.ofEpochSecond(expireDate.getTime()),
					username);
		} catch (IllegalArgumentException iae) {
			throw new AppException(
					"createToken() caught IllegalArgumentException: "
							+ iae.getMessage());
		}
	}
