...
public class Project1 extends Project {	
	...
	public Project1(Connection connection, HttpServletRequest httpRequest, HttpServletResponse httpResponse) {
	...
	public BigInteger decodeBase64(String base64Str) {
		 ...
		 *           and then pass it to new String(). The method getBytes() should
		 *           also define a specific characterset
		 *           
		...
		/**
		 * SOLUTION: Hint for a later milestone, but this direct instantiation of BigInterger() did not perform
		 *           any check on the size of the numeric representation of the string. It is also not catching
		 *           NumberFormatException which could be thrown here. These errors can be fixed for extra credit
		 */
	 ...
	 * 
	 * RISK: Integer overflow can occur with add, subtract, multiply, divide, and other mathematical operations.
	 *       We will cover a few here to provide the foundation for understanding.
	 * 
	...
	public int calcTotalValue(int num) throws AppException {
		...
		/**
		 * SOLUTION: Java 8 added new mathematical features to avoid integer overflow
		 *           Math.addExact() and Math.multiplyExact(). These methods should be used instead
		 *           of basic math operators.
		 *
		 *           The existing code was commented out and replaced with the new Math methods. The
		 *           following import is also need to catch the overflow.
		 *           
	 ...
	 * 
	 * RISK: If the application performs a divide by zero in the operation, it could cause the application
	 *       to fail into an unknown state or crash. A malicious user to create a denial of service attack
	 *       with a simple to perform malicious input.
	 * 
	 ...
	 * 
	 * RISK: The equality check to NaN does not always produce expected results, which could place the check
	 *       into an unknown state. This could allow a malicious attacker to cause instability, denial of service,
	 *       or possibly bypass checks. The method also fails to account for the userInput value being other values such
	 *       as infinity or -infinity.
	 *  
	 ...
	 * 
	 * RISK: String representations of numbers can return unexpected results based on the precision or
	 *       other other factors such as scientific notation. Attempting to compare the string representation
	 *       is not a secure method and can lead to unexpected results or bypasses of checks.
	 * 
	 ...
	 * 
	 * RISK: Easy to predict numbers from weak pseudo random number generators could be exploited by
	 *       malicious actors.
	 * 
	...
	public int randomNumGenerate(int range) throws AppException{
		 ...
		 *           should use SecureRandom instead. SecureRandom requires the import below. In addition,
		 *           Java 8 provides strong instances which requires the import of NoSuchAlgorithmException
		 *           
		 * import java.security.SecureRandom;
         * import java.security.NoSuchAlgorithmException;
		 *
